...
-  保留有<u>连续性</u>的候选关键帧：要求连续3帧关键帧都检测到闭环，且相邻两帧的前10帧共视关键帧有重叠<img src="images/image-20220830202620065.png" alt="image-20220830202620065" style="zoom: 50%;" />

### Compute Sim3

- sim3：相似变换(Similarity Transformation)，旋转R、平移t、尺度因子s。
...
- 步骤
    ...
    - 将 每一个闭环候选帧 与当前关键帧 匹配特征（`SearchByBoW`），保留匹配数>20的 为该帧构造 一个`Sim3Solver`
    
    - 对每一个候选帧用Sim3Solver 迭代匹配，直到有一个候选帧匹配成功（称为闭环关键帧），没有则退出
    
        - 对每一个候选帧的Sim3Solver 迭代求解 相似变换矩阵（Scm）
    
        - 利用Scm 将求解得到的inlier 与当前关键帧 匹配特征（`SearchBySim3`）
    ...
    - 将 闭环关键帧 及其连接关键帧（共视关键帧？） 的所有地图点 投影到 当前关键帧 进行投影匹配（`SearchByProjection`）
...
- Step 2：通过位姿传播，得到Sim3优化后，与当前帧相连的关键帧的位姿，以及它们的地图点
    ...
    - 计算组中每个共视关键帧 在世界坐标系下的Sim3位姿，即g2oSic = g2oSic * mg2oScw
        - g2oSic：由 当前关键帧 到 该共视关键帧 的相对变换矩阵 转换过来
        - mg2oScw：认为是准的，[上个步骤](#Compute Sim3)得到，上图红线
    - 根据 修正后的 每个共视关键帧的Sim3位姿，修正它们的地图点 & 其平均观测方向 & 观测距离范围。每个地图点只根据第一个遍历到它的 共视关键帧进行修正，不会重复修正
...
- Step 5：更新当前关键帧组之间的两级共视相连关系，得到 因闭环时地图点融合 而<u>新得到</u>的连接关系
...
- 本质图，<u>节点</u>是所有关键帧，但<u>边更少</u>，只保留了 联系紧密的边来使得结果更精确
